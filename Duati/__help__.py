import importlib, re
from Duati import dispatcher, ALLOW_EXCL, LOGGER
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, ParseMode, Update
from Duati.Handlers.misc import paginate_modules
...
def admin_help_button(update, context):
    ...
    try:
        if mod_match:
            ...
            text = (
                "Hei hi he *{}* module a ṭanpuina awm te chu ani:\n".format(
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            update.effective_message.reply_text(
                f"He {module.capitalize()} module hrilhfiahna hi i hriat duh chuan PM lam ah min rawn be rawh",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
            ...
            return
        update.effective_message.reply_text(
            "Command hman theih te i hriat duh chuan min rawn PM rawh.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
        ...
        module = args[1].lower()
        text = (
            "Hei hi he *{}* module a i hman theih te chu an ni:\n".format(
                HELPABLE[module].__mod_name__
            )
            + HELPABLE[module].__help__
        )
...
def send_settings(chat_id, user_id, user=False):
    if user:
        if USER_SETTINGS:
            ...
            dispatcher.bot.send_message(
                user_id,
                "Heng te hi settings i siam lai te an ni:" + "\n\n" + settings,
                parse_mode=ParseMode.MARKDOWN,
            )
...
def get_settings(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            ...
            text = "I setting en tur in hmet rawh."
