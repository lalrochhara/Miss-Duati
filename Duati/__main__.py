...
from Duati.Handlers.validation import is_user_admin
...
from Duati.Plugins import ALL_MODULES
from Duati.__help__ import (
get_help, 
help_button, 
get_settings, 
settings_button, 
...
def start(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            if args[0].lower() == "help":
                ...
                send_admin_help(
                    ...
                    ADMIN[mod].__help__,
                    InlineKeyboardMarkup(
                        [[InlineKeyboardButton(text="Kir leh", callback_data="admin_back")]]
                    ),
                )
                send_user_help(
                    update.effective_chat.id,
                    USER[mod].__help__,
                    InlineKeyboardMarkup(
                        [[InlineKeyboardButton(text="Kir leh", callback_data="user_back")]]
                    ),
                )
                send_tools_help(
                    update.effective_chat.id,
                    USER[mod].__help__,
                    InlineKeyboardMarkup(
                        [[InlineKeyboardButton(text="Kir leh", callback_data="tools_back")]]
                    ),
                )
    ...
    [
        InlineKeyboardButton(
            text="Group ah min add rawh", url=f"https://t.me/{context.bot.username}?startgroup=true"),
    ],
    [
    ...
    [
        ...
        InlineKeyboardButton(
            text="Bot Info", callback_data="Duati_"
        ),
    ],
    [
        InlineKeyboardButton(text="Ṭanpuina leh Command", callback_data="help_back"),
    ],
]),
            ...
            text = (
                f"Chibai {mention_html(user.id, user.first_name)}, Kei hi {bot.first_name} ka ni e\n\n"
                f"┏━━━━━━━━━━━━━━━━━━━\n"
                f"┣[• Siamtu : @{OWNER_USERNAME}  \n"
                f"┣[• Uptime : {uptime} \n"
...
def Duati_about_callback(update: Update, context: CallbackContext):
    first_name = update.effective_user.first_name
    query = update.callback_query
    if query.data == "Duati_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                   [
                     InlineKeyboardButton(text="Support", url="t.me/Duati_Support"),
                     InlineKeyboardButton(text="News", url="t.me/Duati_Updates"),
                   ],
                   [
                    InlineKeyboardButton(text="Kir leh", callback_data="Duati_back")
                   ]
                ]
            ),
        )
    elif query.data == "Duati_back":
        query.message.edit_text(
                PM_START_TEXT.format(
                        escape_markdown(first_name), escape_markdown(context.bot.first_name)),
                reply_markup=InlineKeyboardMarkup([
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            stringz = "Min siamtu duhtak hna ka thawk leh thei tawh e. Min siamṭhat leh avangin ka lawm e."
    ...
    about_callback_handler = CallbackQueryHandler(Duati_about_callback, pattern=r"Duati_", run_async=True)
